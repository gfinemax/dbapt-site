...
export function PlanTab() {
  const briefingImages = [
    ...
    },
    {
      ...
      desc: "설명회 자료 10쪽에 수록된 단지 배치도입니다.",
    },
  ];

  const layouts = [
    { title: "101~105동 배치 계획", desc: "설명회 배치도는 101동부터 105동까지의 공동주택 동 배치와 단지 내 보행·차량 동선을 함께 제시합니다." },
    { title: "등용로변 진입 동선", desc: "배치도에는 등용로를 따라 보행자 출입구와 차량 진입구, 주출입구가 구분되어 표시되어 있습니다." },
    { title: "근린생활시설 및 사회복지시설", desc: "단지 동측에는 근린생활시설과 사회복지시설 계획이 함께 표시되어 지역 기반시설과의 연계를 전제로 합니다." },
    { title: "공공보행통로 계획", desc: "단지 중앙부에는 공공보행통로가 표시되어 보행 흐름을 고려한 배치 방향을 확인할 수 있습니다." },
  ];
      ...
      <div className="stone-card bg-white p-6 sm:p-8 rounded-2xl border border-stone-surface">
        <div className="border-b border-stone-surface pb-4 mb-6">
          ...
          <p className="text-xs text-graphite/80 mt-1">
            첨부 설명회 PDF에 수록된 조감도와 배치도를 공개 가능한 범위로 반영했습니다.
          </p>
        </div>
        ...
        <p className="mt-4 rounded-xl bg-[#f8f7f4] px-4 py-3 text-[11px] leading-relaxed text-graphite border border-stone-surface/70">
          위 이미지는 2025.09.06 설명회 자료에서 추출한 참고 이미지이며, 확정 조감도 또는 최종 인허가 도면으로 해석하면 안 됩니다.
        </p>
      </div>
