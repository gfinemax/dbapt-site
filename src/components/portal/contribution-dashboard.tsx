import { CalendarClock, Check, ChevronDown, Database, Home, Layers3, ReceiptText } from "lucide-react";
import { cn } from "@/lib/utils";
import type { ContributionDashboardView, PaymentNoticeView } from "@/lib/contribution-types";

type ContributionDashboardProps = { dashboard: ContributionDashboardView; paymentNotices?: PaymentNoticeView[]; hideSelectedUnit?: boolean };
...
export function ContributionDashboard({ dashboard, paymentNotices = [], hideSelectedUnit = false }: ContributionDashboardProps) {
  ...
  return (
    <article className="stone-card overflow-hidden bg-white p-5 sm:p-7 lg:p-8">
      <div data-testid="contribution-dashboard-layout" className="space-y-7">
        ...
        <section data-testid="contribution-stage-section" className="space-y-4 rounded-[10px] bg-parchment-card p-4 sm:p-5">
          ...
          {dashboard.stageSummary.length > 0 ? (
            <ol aria-label="납부 단계" className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              {dashboard.stageSummary.map((stage) => (
                <li key={stage.id} className="rounded-[10px] bg-white p-3 shadow-[inset_0_0_0_1px_#f2f0ed]">
            ...
            <div className="rounded-[10px] bg-white p-4 shadow-[inset_0_0_0_1px_#f2f0ed]">
              ...
              <ol aria-label="예상 납부 단계" className="mt-4 grid grid-cols-2 gap-2 md:grid-cols-5">
                {commonStageHints.map((stage, index) => <li key={stage.fullLabel} aria-label={stage.fullLabel} className="flex min-h-[104px] min-w-0 flex-col justify-between rounded-[10px] bg-parchment-card px-3 py-3"><span className="inline-flex size-5 items-center justify-center rounded-full bg-ember-orange/10 text-[11px] font-semibold text-ember-orange">{index + 1}</span><span className="block space-y-1"><span className="block break-keep text-sm font-semibold leading-5 text-charcoal-primary">{stage.title}</span>{stage.detail && <span className="block break-keep text-xs leading-5 text-graphite">{stage.detail}</span>}</span></li>)}
              </ol>
            </div>
          )}
            ...
            {dashboard.ledgerEntries.length > 0 ? (
              <details data-testid="contribution-ledger-disclosure" className="group">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 rounded-[10px] px-1 py-1 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-midnight [&::-webkit-details-marker]:hidden">
                  <span className="min-w-0"><h3 className="text-sm font-semibold text-charcoal-primary">최근 납부내역</h3><span className="mt-1 block text-xs text-ash">{dashboard.ledgerEntries.length}건 · 합계 {formatMoney(ledgerTotal)}</span></span>
