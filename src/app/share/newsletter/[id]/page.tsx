...
export async function generateMetadata({ params }: SharePageProps): Promise<Metadata> {
  ...
  try {
    ...
    const sharePath = `/share/newsletter/${encodeURIComponent(news.id)}`;
    ...
    console.error("Newsletter share metadata error:", error);
    return defaultSiteMetadata;
  }
}

export default async function NewsletterSharePage({ params }: SharePageProps) {
  const { id } = await params;
  ...
  return (
    <ShareRedirectPage
      ...
      destination={`/news?tab=newsletter&news=${encodeURIComponent(id)}`}
    />
  );
}
